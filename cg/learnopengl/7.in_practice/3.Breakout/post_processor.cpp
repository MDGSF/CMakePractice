    ...
    Shake(false)
{
  ...
  if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
    std::cout << "ERROR:POSTPROCESSOR: Failed to init FBO" << std::endl;
  }
  ...
  glUniform1iv(glGetUniformLocation(this->PostProcessingShader.ID, "edge_kernel"),
      9, edge_kernel);
  ...
  glUniform1fv(glGetUniformLocation(this->PostProcessingShader.ID, "blur_kernel"),
      9, blur_kernel);
}
